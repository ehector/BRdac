    ...
    for(int s_1=0; s_1<S-1; s_1++){
      for(int s_2=s_1+1; s_2<S; s_2++){
        ...
        if(x_1 > u & x_2 <= u){
          ...
          score_i(0) += -A_k1_alpha * exp(-pow(A_k1, 2.0)/2) /(x_1 * sqrt_2_pi) -A_k2_alpha * exp(-pow(A_k2, 2.0)/2) /(u * sqrt_2_pi) +
            (1/V_x1)*(norm_A_k1*(a_12_alpha/pow(a_12, 2.0) + A_k1*A_k1_alpha/a_12-A_k1_alpha)/x_1_sq -
            norm_A_k2*(a_12_alpha/a_12 + A_k2*A_k2_alpha)/(a_12*x_1*u) );
          
          score_i(1) += -A_k1_phi * exp(-pow(A_k1, 2.0)/2) /(x_1 * sqrt_2_pi) -A_k2_phi * exp(-pow(A_k2, 2.0)/2) /(u * sqrt_2_pi) +
            (1/V_x1)*(norm_A_k1*(a_12_phi/pow(a_12, 2.0) + A_k1*A_k1_phi/a_12-A_k1_phi)/x_1_sq -
        ...
        if(x_1 <= u & x_2 > u){
          ...
          score_i(0) += -A_k1_alpha * exp(-pow(A_k1, 2.0)/2) /(u * sqrt_2_pi) -A_k2_alpha * exp(-pow(A_k2, 2.0)/2) /(x_2 * sqrt_2_pi) +
            (1/V_x2)*(norm_A_k2*(a_12_alpha/pow(a_12, 2.0) + A_k2*A_k2_alpha/a_12- A_k2_alpha)/x_2_sq -
                  norm_A_k1*(a_12_alpha/a_12 + A_k1*A_k1_alpha)/(a_12*u*x_2) );
          
          score_i(1) += -A_k1_phi * exp(-pow(A_k1, 2.0)/2) /(u * sqrt_2_pi) -A_k2_phi * exp(-pow(A_k2, 2.0)/2) /(x_2 * sqrt_2_pi) +
            (1/V_x2)*(norm_A_k2*(a_12_phi/pow(a_12, 2.0) + A_k2*A_k2_phi/a_12- A_k2_phi)/x_2_sq -
...
double logCL_all_thresh(const arma::vec& par, const arma::vec& thresholds, const arma::mat& y, 
  ...
  for(int s_1=0; s_1<S-1; s_1++){
    for(int s_2=s_1+1; s_2<S; s_2++){
      ...
      SUM += logPCL_all_thresh(alpha, phi, beta_1, beta_2, beta_3, thresholds(s_1), thresholds(s_2), join_rows(y.col(s_1), y.col(s_2)), 
