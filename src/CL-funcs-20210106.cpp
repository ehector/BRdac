...
// [[Rcpp::export]]
double logCL_BR(const arma::vec& par, const arma::mat& x, const arma::mat& locs){
...
arma::mat Cscore_all(const arma::vec& par, const arma::mat& y, const arma::cube& z_1, const arma::cube& z_2, const arma::cube& z_3, const arma::mat& locs){
  ...
  const arma::vec& beta_2 = par.subvec(2+p_1, 1+p_1+p_2);
  const arma::vec& beta_3 = par.subvec(2+p_1+p_2, 1+p_1+p_2+p_3);
  arma::mat output = zeros<mat>(2+p_1+p_2+p_3, n);
...
arma::mat Chessian_all_sum(const arma::vec& par, const arma::mat& y, const arma::cube& z_1, const arma::cube& z_2, const arma::cube& z_3, 
  ...
  return sensitivity*sensitivity.t();
}

// [[Rcpp::export]]
List Chessian_all_raw(const arma::vec& par, const arma::mat& y, const arma::cube& z_1, const arma::cube& z_2, const arma::cube& z_3, 
                           const arma::mat& locs){
  static const double pi = 3.14159265359; 
  static const double sqrt_2_pi = pow(2*pi, 0.5);
...
struct quadratic_form_worker : public Worker
{
  // source
  const arma::vec par; 
  const arma::mat y;
  ...
  quadratic_form_worker(const arma::vec& par, const arma::mat& y, const arma::cube& z_1, const arma::cube& z_2,
                        const arma::cube& z_3, const arma::mat& locs, const int& K, arma::mat& psi) 
    : par(par), y(y), z_1(z_1), z_2(z_2), z_3(z_3), locs(locs), K(K), psi(psi) {}
  
  void operator()(std::size_t begin, std::size_t end) {
    
    const int p = par.size();
    const int s_k = y.n_cols/K;
    
    for(unsigned int i=begin; i < end; i++) {
      int k = static_cast<int>(i);
      psi.rows(k*p, (k+1)*p-1) = Cscore_all(par, y.cols(k*s_k, (k+1)*s_k-1), z_1.rows(k*s_k, (k+1)*s_k-1), z_2.rows(k*s_k, (k+1)*s_k-1), 
               z_3.rows(k*s_k, (k+1)*s_k-1), locs.rows(k*s_k, (k+1)*s_k-1));
    }
  }
};

// [[Rcpp::export]]
double quadratic_form_par(const arma::vec& par, const arma::mat& y, const arma::cube& z_1, const arma::cube& z_2, const arma::cube& z_3, 
                          const arma::mat& locs, const int& K){
  const arma::vec one = ones<vec>(y.n_rows);
  arma::mat psi = zeros<mat>(par.size()*K, y.n_rows);
  
  // quadratic_form_par functor (pass input and output matrices)
  quadratic_form_worker little_worker(par, y, z_1, z_2, z_3, locs, K, psi);
  
  // call parallelFor to do the work
  parallelFor(0, K, little_worker); 
  
  arma::vec psi_sum = psi * one;
  double output = as_scalar( psi_sum.t() * inv(psi * psi.t()) * psi_sum );
  
  if(std::isnan(output)) output = R_PosInf;
  return output;
}
