BRdac <- function(y, covariates_1, covariates_2, covariates_3, u, locs, indicator){
  time <- proc.time()
  output <- list()
  ...
  for(k in 1:K){
    ...
    result_k <- Chessian_all_thresh(mean_estimates, u=u, y=y[,indicator==k], 
...
BRdac_BR <- function(x, locs, indicator){
  ...
  for(k in 1:K){
    ...
    opt_2 <- nlminb(start=init_values, objective=logCL_BR_thresh, u=u, x=x[,indicator==k], locs=locs[indicator==k,], lower=c(1e-5,1e-5), upper=c(2-1e-5,Inf))
