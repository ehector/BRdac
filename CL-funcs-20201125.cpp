                 ...
                 const arma::vec& loc_1, const arma::vec& loc_2)
{
  ...
  for(int i=0; i<n; i++){
    ...
    J_1 =1/sigma_1 * pow(1 + xi_1 * (y(i,0) - mu_1) / sigma_1, 1 / xi_1);
    J_2 =1/sigma_2 * pow(1 + xi_2 * (y(i,1) - mu_2) / sigma_2, 1 / xi_2);
             ...
             const arma::vec& loc_1, const arma::vec& loc_2)
{
  ...
  for(int i=0; i<n; i++){
    ...
    score_i.rows(2,p_1+1) = 
      -1/sigma_1 * pow(cent_1, -1) * z_1_1.row(i).t() 
      -1/sigma_2 * pow(cent_2, -1) * z_1_2.row(i).t()
    ...
    score_i.rows(p_1+2, p_1+p_2+1) = 
      -( 1+ (y(i,0) - mu_1)*pow(cent_1, -1)/sigma_1 ) * z_2_1.row(i).t()
      -( 1+ (y(i,1) - mu_2)*pow(cent_2, -1)/sigma_2 ) * z_2_2.row(i).t()
    ...
    score_i.rows(p_1+p_2+2, p_1+p_2+p_3+1) = 
      1/xi_1*( (y(i,0) - mu_1)/sigma_1 /( cent_1 ) -
              1/xi_1 * log(cent_1 )
      ) * z_3_1.row(i).t() +
      1/xi_2*( (y(i,1) - mu_2)/sigma_2 /( cent_2 ) -
              1/xi_2 * log(cent_2 )
      ) * z_3_2.row(i).t()
