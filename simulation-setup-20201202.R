...
phi <- 3
N <- 7
s <- as.matrix(expand.grid(1:3,1:3))
S <- nrow(s)
p_1 <- p_2 <- p_3 <- 2
beta_1 <- c(0.5,0.5)
beta_2 <- c(1,1)
...
invisible(capture.output(R <- RFsimulate(mod, n=N, x=s[,1], y=s[,2])))
...
covariates <- do.call(rbind, lapply(1:N, function(x) return(s))) #matrix(rnorm(N*S*p, 0, 1), N*S, p)
...
opt_2 <-
  nlminb(start=c(alpha, phi,beta_1,beta_2,beta_3), objective=logCL_all_wrap, gradient=Cscore_all_sum_wrap, 
